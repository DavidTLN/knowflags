...
function StatCard({ icon, value, label, accent }) {
  return (
    <div style={{ backgroundColor: C.white, borderRadius: '14px', padding: '20px 16px', border: `1px solid ${C.border}`, textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      <div style={{ fontSize: '24px', marginBottom: '6px' }}>{icon}</div>
      <div style={{ fontSize: '26px', fontWeight: '800', color: accent || C.navy, fontFamily: 'var(--font-display)', lineHeight: 1, marginBottom: '5px' }}>{value ?? '—'}</div>
      <div style={{ fontSize: '10px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '1px', color: C.muted }}>{label}</div>
    </div>
  )
}
...
export default function ProfilePage() {
  ...
  // game_scores_log doesn't have flag_code/won — use player_stats for mastered count
  const masteredCount = stats.reduce((a, s) => a + (s.flags_found || 0), 0)
  ...
  // ── saveProfile — only updates columns that exist in profiles ──
  async function saveProfile() {
    ...
    if (!error) {
      ...
      setSaveMsg(t('Error saving. Please try again.', 'Erreur lors de la sauvegarde.'))
    }
    setSaving(false)
  }
  ...
  if (!user) return (
    <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: C.bg, padding: '24px' }}>
      <div style={{ textAlign: 'center', maxWidth: '320px' }}>
        ...
        <a href={`/${locale}/login`} style={{ display: 'inline-block', padding: '12px 28px', backgroundColor: C.navy, color: C.cream, borderRadius: '10px', textDecoration: 'none', fontWeight: '700', fontSize: '14px' }}>
  ...
  return (
    ...
    <div style={{ backgroundColor: C.bg, minHeight: '100vh', fontFamily: 'var(--font-body)' }}>
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '22px', marginBottom: '28px', flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative', cursor: 'pointer' }} onClick={() => fileRef.current?.click()}>
              ...
              <div style={{ position: 'absolute', bottom: '-2px', right: '-2px', width: '24px', height: '24px', borderRadius: '50%', backgroundColor: C.blue, border: `2px solid ${C.navy}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px' }}>
                {avatarBusy ? '⏳' : '✏️'}
              </div>
            </div>
            <div style={{ flex: 1, minWidth: '180px' }}>
              ...
              <div style={{ fontSize: '12px', color: 'rgba(244,241,230,0.45)', marginTop: '4px' }}>{user.email}</div>
              <div style={{ display: 'flex', gap: '7px', marginTop: '10px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '11px', backgroundColor: 'rgba(224,124,58,0.2)', color: '#f0a060', padding: '3px 10px', borderRadius: '99px', fontWeight: '700' }}>🔥 ×{curStreak}</span>
                <span style={{ fontSize: '11px', backgroundColor: 'rgba(66,106,90,0.25)', color: '#7ec4a7', padding: '3px 10px', borderRadius: '99px', fontWeight: '700' }}>✅ {masteredCount} {t('flags found', 'drapeaux trouvés')}</span>
                <span style={{ fontSize: '11px', backgroundColor: 'rgba(128,109,64,0.25)', color: '#d4a96a', padding: '3px 10px', borderRadius: '99px', fontWeight: '700' }}>⭐ {totalScore.toLocaleString()} pts</span>
              </div>
            </div>
            <button onClick={startEdit} style={{ padding: '8px 16px', backgroundColor: 'rgba(255,255,255,0.1)', color: C.cream, border: '1px solid rgba(255,255,255,0.18)', borderRadius: '9px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', whiteSpace: 'nowrap' }}>
              ✏️ {t('Edit profile', 'Modifier le profil')}
            </button>
          </div>
          <div style={{ display: 'flex', overflowX: 'auto' }}>
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '11px 18px', border: 'none', background: 'transparent', fontSize: '13px', fontWeight: '600', cursor: 'pointer', whiteSpace: 'nowrap', color: activeTab === tab.id ? C.cream : 'rgba(244,241,230,0.4)', borderBottom: activeTab === tab.id ? `2px solid ${C.blue}` : '2px solid transparent', transition: 'all 0.15s' }}>
                {t(tab.en, tab.fr)}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        {activeTab === 'overview' && (<>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '36px' }}>
            {Object.entries(GAMES_META).map(([key, meta]) => {
              ...
              return (
                <div key={key} style={{ backgroundColor: C.white, borderRadius: '13px', padding: '16px 20px', border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: '14px', boxShadow: '0 2px 6px rgba(0,0,0,0.04)' }}>
                  <span style={{ fontSize: '26px', width: '34px', textAlign: 'center', flexShrink: 0 }}>{meta.icon}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '14px', fontWeight: '700', color: C.navy, marginBottom: '5px', fontFamily: 'var(--font-display)' }}>{t(meta.en, meta.fr)}</div>
                  ...
                  {(s?.streak_current > 0) && (
                    <span style={{ fontSize: '12px', backgroundColor: '#fff4e8', color: '#e07c3a', padding: '4px 10px', borderRadius: '99px', fontWeight: '700', flexShrink: 0 }}>🔥 ×{s.streak_current}</span>
                  )}
                </div>
              )
            })}
            ...
            : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {history.slice(0, 15).map((h, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: C.white, borderRadius: '9px', padding: '10px 14px', border: `1px solid ${C.border}` }}>
                      <span style={{ fontSize: '22px', width: '32px', textAlign: 'center', flexShrink: 0 }}>{meta.icon || '🎮'}</span>
                      <span style={{ fontSize: '12px', color: C.muted, flex: 1 }}>{t(meta.en, meta.fr) || h.game}</span>
                  ...
                  {key !== 'capital-city' ? (
                    ...
                    <div style={{ padding: '16px 20px' }}>
                      {capitalModeScores.length === 0
                        ? <p style={{ color: C.muted, fontSize: '14px', textAlign: 'center', margin: '12px 0' }}>{t('No games played yet', 'Aucune partie jouée')}</p>
                        : (
                            ...
                            {capitalModeScores.map(gs => {
                              ...
                              return (
                                <div key={gs.mode} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', backgroundColor: C.bg, borderRadius: '10px', border: `1px solid ${C.border}` }}>
                                  <span style={{ fontSize: '13px', fontWeight: '600', color: C.navy }}>{modeLabel ? t(modeLabel.en, modeLabel.fr) : gs.mode}</span>
                                  <span style={{ fontSize: '13px', color: C.muted }}>🔥 <strong style={{ color: '#e07c3a' }}>{gs.best_streak}</strong> {t('best streak', 'meilleure série')}</span>
                                </div>
                              )
                            })}
                          </div>
            ...
            : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {history.slice(0, 50).map((h, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '13px', backgroundColor: C.white, borderRadius: '10px', padding: '11px 15px', border: `1px solid ${C.border}` }}>
                      <span style={{ fontSize: '22px', width: '32px', textAlign: 'center', flexShrink: 0 }}>{meta.icon || '🎮'}</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: C.navy }}>{t(meta.en, meta.fr) || h.game}</div>
                        <div style={{ fontSize: '11px', color: C.muted }}>{date} {time}</div>
                      </div>
        ...
        {/* FLAGS */}
        {activeTab === 'flags' && (<>
          <SectionTitle>✅ {masteredCount} {t('flags found', 'drapeaux trouvés')}</SectionTitle>
          <div style={{ padding: '48px 0', textAlign: 'center' }}>
            <div style={{ fontSize: '42px', marginBottom: '14px' }}>🏳️</div>
            <div style={{ fontSize: '15px', color: C.muted, marginBottom: '8px' }}>
              {masteredCount > 0
                ? t(`You've found ${masteredCount} flags across all games!`, `Tu as trouvé ${masteredCount} drapeaux dans tous les jeux !`)
                : t('Play games to build your flag collection!', 'Jouez pour constituer votre collection de drapeaux !')}
            </div>
        ...
        {activeTab === 'badges' && (<>
          ...
          {unlocked.length === 0
            ...
            : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px', marginBottom: '36px' }}>
                {unlocked.map(b => (
                  <div key={b.id} style={{ backgroundColor: C.white, borderRadius: '12px', padding: '16px', border: `1.5px solid ${C.green}`, boxShadow: '0 2px 8px rgba(66,106,90,0.1)', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
            {locked.map(b => (
              <div key={b.id} style={{ backgroundColor: C.white, borderRadius: '12px', padding: '16px', border: `1px solid ${C.border}`, opacity: 0.45, display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ maxWidth: '480px' }}>
            ...
            <div style={{ backgroundColor: C.white, borderRadius: '16px', border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: '20px' }}>
              ...
              <div style={{ padding: '20px 22px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: '14px' }}>
                ...
                <button onClick={() => fileRef.current?.click()} style={{ padding: '7px 14px', border: `1px solid ${C.border}`, borderRadius: '8px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', backgroundColor: C.bg, color: C.navy }}>
              ...
              <div style={{ padding: '20px 22px', borderBottom: `1px solid ${C.border}` }}>
                ...
                <input
                  ...
                  placeholder={t('Your username', 'Votre nom')}
                  style={{ width: '100%', boxSizing: 'border-box', padding: '9px 13px', borderRadius: '8px', border: `1.5px solid ${editing ? C.blue : C.border}`, fontSize: '14px', color: C.navy, backgroundColor: C.white, outline: 'none' }}
                />
              </div>

              {/* Bio */}
              <div style={{ padding: '20px 22px', borderBottom: `1px solid ${C.border}` }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: C.navy, marginBottom: '8px' }}>Bio</label>
                <textarea
                  value={editing ? editForm.bio : (profile?.bio || '')}
                  onChange={e => setEditForm(f => ({ ...f, bio: e.target.value }))}
                  onFocus={() => !editing && startEdit()}
                  placeholder={t('A short bio…', 'Une courte bio…')}
                  rows={3}
                  style={{ width: '100%', boxSizing: 'border-box', padding: '9px 13px', borderRadius: '8px', border: `1.5px solid ${editing ? C.blue : C.border}`, fontSize: '14px', color: C.navy, backgroundColor: C.white, outline: 'none', resize: 'vertical' }}
                />
              </div>
              ...
              <div style={{ padding: '20px 22px' }}>
                ...
                <div style={{ fontSize: '11px', color: C.muted, marginTop: '5px' }}>
                  {t('Email cannot be changed here.', "L'email ne peut pas être modifié ici.")}
                </div>
              </div>
            </div>
            ...
            {editing && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '32px' }}>
                <button onClick={saveProfile} disabled={saving} style={{ padding: '11px 26px', backgroundColor: C.navy, color: C.cream, border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }}>
                  {saving ? t('Saving…', 'Enregistrement…') : t('Save changes', 'Enregistrer')}
                </button>
                <button onClick={() => setEditing(false)} style={{ padding: '11px 18px', backgroundColor: 'transparent', color: C.muted, border: `1px solid ${C.border}`, borderRadius: '10px', fontSize: '14px', cursor: 'pointer' }}>
                  {t('Cancel', 'Annuler')}
                </button>
                {saveMsg && <span style={{ fontSize: '13px', color: saveMsg.includes('Error') || saveMsg.includes('Erreur') ? C.red : C.green, fontWeight: '600' }}>{saveMsg}</span>}
              </div>
            )}

            {/* Danger zone */}
            <div style={{ padding: '18px 20px', borderRadius: '12px', border: '1px solid rgba(192,57,43,0.2)', backgroundColor: 'rgba(192,57,43,0.02)' }}>
              <div style={{ fontSize: '12px', fontWeight: '700', color: C.red, marginBottom: '5px' }}>{t('Danger zone', 'Zone dangereuse')}</div>
              ...
              <button style={{ padding: '7px 14px', backgroundColor: 'transparent', color: C.red, border: `1px solid ${C.red}`, borderRadius: '7px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>
