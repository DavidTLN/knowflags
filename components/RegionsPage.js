...
export default function RegionsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [flags, search, activeContinent, activeCountry, activeColors, activeSymbols, activeRatios, activeShapes, sortOrder, locale])
  ...
  const grid = (cols) => (
    filtered.length === 0 ? (
      <div style={{ textAlign: 'center', padding: '48px 0', color: DS.light }}>
        ...
        <p style={{ fontSize: '15px', fontWeight: '600' }}>{t('No regions match your filters', 'Aucune région ne correspond aux filtres')}</p>
      </div>
    ) : (
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: isMobile ? '20px 16px' : '40px 32px' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          <h1 style={{ fontSize: isMobile ? '28px' : '40px', fontWeight: '900', color: DS.navy, margin: '0 0 4px', letterSpacing: '-1px' }}>
            {t('Regional Flags', 'Drapeaux des régions')}
          </h1>
          <p style={{ margin: 0, fontSize: '14px', color: DS.muted }}>
            {filtered.length} {t('regions', 'régions')}
          </p>
        </div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <div style={{ position: 'relative', flex: 1 }}>
                  ...
                  <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                    placeholder={t('Search a region…', 'Rechercher une région…')}
