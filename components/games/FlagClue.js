...
export default function FlagClue() {
      ...
      <div style={{ backgroundColor: '#F4F1E6', height: 'calc(100dvh - 60px)', fontFamily: 'var(--font-body, system-ui)', display: 'flex', flexDirection: 'column' }}>
            ...
            {loading ? (
              ...
              <>
                <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                  ...
                  <h1 style={{ fontSize: '26px', fontWeight: '900', color: '#0B1F3B', margin: '0 0 6px', letterSpacing: '-1px' }}>FlagClue</h1>
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '14px 16px' }}>
                  <div style={{ fontSize: '11px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px' }}>{t('Rules', 'Règles')}</div>
                  {[
        ...
        {!loading && (
          <div style={{ padding: '12px 16px', paddingBottom: 'max(12px, env(safe-area-inset-bottom))', background: '#F4F1E6', borderTop: '1px solid #e2e8f0' }}>
            <button onClick={() => startGame(pendingMode)} style={{ width: '100%', padding: '16px', borderRadius: '14px', backgroundColor: '#0B1F3B', color: 'white', fontSize: '16px', fontWeight: '900', border: 'none', cursor: 'pointer', letterSpacing: '-0.3px' }}>
      ...
      <div style={{ backgroundColor: '#F4F1E6', height: 'calc(100dvh - 60px)', fontFamily: 'var(--font-body, system-ui)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flexShrink: 0, padding: '20px 16px 0' }}>
          <div style={{ maxWidth: '480px', margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
              ...
              <p style={{ margin: 0, color: '#94a3b8', fontSize: '13px' }}>{total} {t('questions', 'questions')}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '12px' }}>
              {[
                { label: t('Correct', 'Correctes'), value: `${correctCount}/${total}`, color: '#426A5A', bg: '#f0fdf4', border: '#bbf7d0' },
                { label: t('Accuracy', 'Précision'), value: `${pct}%`,                color: '#0B1F3B', bg: 'white',   border: '#e2e8f0' },
            ...
            <div style={{ backgroundColor: '#f0fdf4', borderRadius: '14px', border: '1px solid #bbf7d0', padding: '14px 12px', textAlign: 'center', marginBottom: '12px' }}>
              ...
              <div style={{ fontSize: '10px', fontWeight: '700', color: '#94a3b8', marginTop: '4px', textTransform: 'uppercase', letterSpacing: '0.6px' }}>pts</div>
            </div>
          </div>
        </div>
        ...
        {wrong.length > 0 && (
          <div style={{ flex: 1, minHeight: 0, padding: '0 16px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ maxWidth: '480px', margin: '0 auto', width: '100%', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', backgroundColor: 'white', borderRadius: '14px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  {wrong.map((h, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 8px', borderRadius: '10px', backgroundColor: i % 2 === 0 ? '#fafafa' : 'white' }}>
                      <img src={`https://flagcdn.com/w40/${h.country.iso_code}.png`} style={{ height: '20px', borderRadius: '3px', border: '1px solid #e2e8f0' }} alt="" />
                      <span style={{ fontSize: '13px', color: '#0B1F3B', fontWeight: '700', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <div style={{ flexShrink: 0, padding: '12px 16px', paddingBottom: 'max(12px, env(safe-area-inset-bottom))', background: '#F4F1E6', borderTop: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button onClick={() => startGame(mode)} style={{ width: '100%', padding: '16px', backgroundColor: '#0B1F3B', color: 'white', border: 'none', borderRadius: '14px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', letterSpacing: '-0.3px' }}>
  ...
  const optionsEl = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {question.options.map((opt, i) => {
        ...
        let border = answered ? (isThis ? '#22c55e' : '#e2e8f0') : '#e2e8f0'
        let color  = answered ? (isThis ? '#15803d' : '#94a3b8') : '#0B1F3B'
        if (isMobile) {
  ...
  if (isMobile) {
    return (
      <div style={{ backgroundColor: '#0B1F3B', height: 'calc(100dvh - 60px)', fontFamily: 'var(--font-body, system-ui)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '10px 14px 6px', gap: '8px', flexShrink: 0 }}>
          ...
          <div style={{ backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: '20px', padding: '3px 12px', fontSize: '14px', fontWeight: '900', color: timerColor, minWidth: '44px', textAlign: 'center', fontVariantNumeric: 'tabular-nums' }}>{timer}s</div>
            <button onClick={() => setShowQuitConfirm(true)} style={{ width: '30px', height: '30px', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>✕</button>
        </div>
  ...
  // ── DESKTOP ──────────────────────────────────────────────────────────────────
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F4F1E6', fontFamily: 'var(--font-body, system-ui)', paddingTop: '28px', paddingBottom: '48px' }}>
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {streak >= 2 && (
            <div style={{ backgroundColor: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: '8px', padding: '4px 10px', fontSize: '13px', fontWeight: '700', color: '#EA580C' }}>🔥 ×{streak}</div>
          )}
        </div>
          ...
          {isClueMode ? (
            <>
              ...
              {answered && (
                <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', backgroundColor: answered === 'correct' ? '#f0fdf4' : '#fef2f2', borderRadius: '10px', border: `1px solid ${answered === 'correct' ? '#bbf7d0' : '#fecaca'}` }}>
                  ...
                  <div>
                    <div style={{ fontSize: '15px', fontWeight: '900', color: '#0B1F3B' }}>{locale === 'fr' ? question.correct.name_fr : question.correct.name_en}</div>
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <img src={`https://flagcdn.com/w160/${question.correct.iso_code}.png`} alt="" style={{ height: '52px', borderRadius: '6px', objectFit: 'cover', border: '1px solid #e2e8f0', flexShrink: 0 }} />
                <div style={{ fontSize: '20px', fontWeight: '900', color: '#0B1F3B' }}>{locale === 'fr' ? question.correct.name_fr : question.correct.name_en}</div>
              </div>
            </>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {question.options.map((opt, i) => {
            ...
            if (answered) {
              ...
              else { bg = 'white'; border = '#e2e8f0'; color = '#94a3b8' }
            }
            return (
              <button key={i} onClick={() => handleAnswer(opt)} disabled={!!answered}
