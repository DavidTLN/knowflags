        ...
        <div style={{ background: colors.card, borderRadius: '20px', padding: '40px', maxWidth: '480px', width: '100%', boxShadow: '0 8px 32px rgba(11,31,59,0.12)' }}>
          ...
          <div style={{ marginBottom: '28px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              {[
                { key: 'easy',    icon: '🟢', en: 'Easy',    fr: 'Facile',    desc: 'Lines + flag name' },
                { key: 'medium',  icon: '🟡', en: 'Medium',  fr: 'Moyen',     desc: 'Lines only' },
                { key: 'hard',    icon: '🟠', en: 'Hard',    fr: 'Difficile', desc: 'No lines' },
                { key: 'extreme', icon: '🔴', en: 'Extreme', fr: 'Extrême',   desc: 'No lines, no name' },
              ].map(d => (
                <button key={d.key} onClick={() => setDifficulty(d.key)} style={{
                  ...
                  <div style={{ fontSize: '11px', opacity: 0.7, marginTop: '2px' }}>{d.desc}</div>
                </button>
              ))}
            </div>
          </div>
