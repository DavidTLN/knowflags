...
export default function FlagQuiz() {
  ...
  async function quitGame() {
    ...
    await saveStats(history.filter(h => h.isCorrect).length, history.length, Math.max(bestStreak, streakRef.current))
  ...
  async function saveStats(correct, total, bestStreakVal) {
    if (!user) return
    const supabase = createClient()
    const { data: existing } = await supabase.from('player_stats').select('*').eq('user_id', user.id).eq('game', 'flag-quiz').single()
    if (existing) {
      await supabase.from('player_stats').update({
        game: 'flag-quiz', games_played: (existing.games_played || 0) + 1,
        flags_found: (existing.flags_found || 0) + correct,
        streak_best: Math.max(existing.streak_best || 0, bestStreakVal),
        streak_current: 0, updated_at: new Date().toISOString(),
      }).eq('user_id', user.id).eq('game', 'flag-quiz')
    } else {
      await supabase.from('player_stats').insert({ user_id: user.id, game: 'flag-quiz', games_played: 1, flags_found: correct, streak_best: bestStreakVal, streak_current: 0 })
    }
  }
  ...
  async function logScore(score) {
    ...
    try {
      ...
      if (!user) return
      await supabase.from('game_scores_log').insert({ user_id: user.id, game: 'flag-quiz', score, played_at: new Date().toISOString() })
  ...
  async function saveScore(finalScore, bestStreakVal) {
    ...
    const supabase = createClient()
    const { data: existing } = await supabase.from('game_scores').select('best_streak').eq('user_id', user.id).eq('mode', 'flag_quiz').single()
    await supabase.from('game_scores').upsert({
      user_id: user.id, mode: 'flag_quiz',
  ...
  useEffect(() => {
    ...
    supabase.from('countries').select('iso_code, name_en, name_fr, region').order('name_en')
      ...
      .catch(() => { clearTimeout(timeout); setCountriesLoading(false) })
  ...
  const makeNextQuestion = useCallback(() => {
    setQuestion(buildQuestion(getPool(), pickMode()))
    setAnswered(null)
  ...
  function startGame() {
    ...
    streakRef.current = 0
    scoreRef.current = 0
    setLives(MAX_LIVES); setStreak(0); setBestStreak(0)
    setScore(0); setBestScore(0); setElapsed(0); setLastPts(null); setHistory([])
  ...
  function handleAnswer(selected) {
    ...
    if (isCorrect) {
      ...
      streakRef.current = ns; setStreak(ns); setBestStreak(prev => Math.max(prev, ns))
      const pts = Math.round((POINTS_CORRECT + timer * POINTS_TIMER_BONUS) * STREAK_MULTIPLIER(ns))
      const newScore = scoreRef.current + pts
      scoreRef.current = newScore; setScore(newScore); setBestScore(b => Math.max(b, newScore))
